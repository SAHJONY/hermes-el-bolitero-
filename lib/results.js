...
//                       use (default: florida,georgia,chicago,pr). With only those
//                       4 boards (~18 game codes) steady-state is ≈18 calls/day
//                       (~540/month) — comfortably inside Silver for a full month.
//                       New York is free/unmetered; all other boards show DEMO.
// ─────────────────────────────────────────────────────────────────────────────
...
// Puerto Rico); New York is free/unmetered and always on regardless. Every other
// board falls back to the clearly-labeled DEMO engine — zero API calls. Widen the
// roster later WITHOUT a code change by setting MAGAYO_BOARDS (comma list of ids),
// or MAGAYO_BOARDS="all" to poll every mapped board.
const BOARDS_ALLOW = cleanEnv(process.env.MAGAYO_BOARDS || "florida,georgia,chicago,pr")
  .split(",").map((s) => s.trim()).filter(Boolean);
