...
//                       use (default: florida,georgia,chicago,pr,texas). Those 5
//                       boards = ~26 game codes → ≈26 calls/day (~780/mo worst
//                       case), comfortably under the 950 cap. Texas is the heavy
//                       one (4 draws/day = 8 codes). New York is free/unmetered;
//                       all other boards show DEMO.
// ─────────────────────────────────────────────────────────────────────────────
...
// Puerto Rico, Texas); New York is free/unmetered and always on regardless. Every
// other board falls back to the clearly-labeled DEMO engine — zero API calls.
// Widen/trim the roster WITHOUT a code change by setting MAGAYO_BOARDS (comma list
// of ids), or MAGAYO_BOARDS="all" to poll every mapped board.
const BOARDS_ALLOW = cleanEnv(process.env.MAGAYO_BOARDS || "florida,georgia,chicago,pr,texas")
  .split(",").map((s) => s.trim()).filter(Boolean);
...
async function buildResults(opts = {}) {
  ...
  // La Bolita mirrors Florida's REAL draw — the classic Cuban bolita reads off
  // the Florida numbers — reusing Florida's already-fetched result at ZERO extra
  // API cost. Only the sessions Florida actually draws (Mediodía/Noche) turn real;
  // La Bolita's Mañana/Tarde have no Florida source and stay labeled demo.
  if (boardAllowed("florida")) {
    for (const d of real.filter((x) => x.board === "florida")) {
      real.push({ board: "labolita", date: d.date, session: d.session, pick3: d.pick3, pick4: d.pick4, source: "real" });
      realBoards.add("labolita");
    }
  }
